...
function highlightColor(i, round) {
  d3.selectAll("circle.graphNode").attr("class", (o) =>
    o.crtree[round].rank === i
      ? "graphNode highlight"
      : "graphNode nonhighlight",
  )
  d3.selectAll("line.graphEdge").attr("class", (o) =>
    o.source.crtree[round].rank === i && o.target.crtree[round].rank === i
      ? "graphEdge highlight"
      : "graphEdge nonhighlight",
  )
...
async function drawTrees(state, trees) {
  ...
  for (let i = 0; i < trees.length; i++) {
    ...
    div.on("mouseover", () => {
      hoveringTreeRound = state.round
      highlightColor(i, state.round)
    })
    div.on("mouseout", () => {
...
async function reload(forceResample = false) {
  ...
  if (changedFields.has("round")) {
    ...
    d3.selectAll("circle.graphNode").attr("fill", (v) => {
      return color(
        v.crtree[state.round].rank,
        treesPerRound[state.round].length,
        state.round,
        treesPerRound.length,
      )
    })
...
function drawNavElements(state) {
  ...
  document.getElementById("crtrees").style.display = state.crtrees
    ? "grid"
    : "none"
...
function main() {
  ...
  document
    ...
    .addEventListener("click", () => increase("round"))
  document
    .getElementById("left")
    .addEventListener("click", () => decrease("round"))
  document
    .getElementById("reload")
    .addEventListener("click", () => reload(true))
  document
    .getElementById("count")
    .addEventListener("click", () => toggle("count"))
